...
- 已支持：点击、长按、滑动、键盘输入、等待、识图、查找文字、识别区域文字、变量、循环、无限循环、条件、启动应用、返回、主页、停止运行、日志
- 未纳入首版：手势录制、Root/Shizuku、MediaProjection

识图图片会统一放在脚本工作目录的 `Images` 子文件夹下，脚本中只需要填写图片文件名。`识图` 作为表达式使用时会返回结果对象，可读取 `找到`、`x`、`y`、`置信度`。
文字识别使用 ML Kit bundled 中文模型在本机处理截图。`查找文字` 可全屏或按区域查找目标文字并返回 `找到`、`文本`、`x`、`y`、`左`、`上`、`右`、`下`；`识别文字` 用于指定区域 OCR，可读取 `找到`、`文本`、`行数`。
